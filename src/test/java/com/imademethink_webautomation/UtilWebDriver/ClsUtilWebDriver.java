...
public class ClsUtilWebDriver extends ClsGeneralInitTerminateAndOtherUtil{
	...
	public static WebDriver instanciate_Web_Driver() {
		...
		// if browser type is random
		if (strGlobalDriverType.contains("random")) {
			Random objRand 				= new Random();
			int nRandomIndex 			= objRand.nextInt(arrStrDriverTypeNames.length);
			strGlobalDriverType 		= arrStrDriverTypeNames[nRandomIndex];
		}
		
		int nnDriverInitTryCount = 0;
		for (; nnDriverInitTryCount < 3 ; nnDriverInitTryCount++){
			try{
				if (null == REAL_WEB_DRIVER) {
					if (strGlobalDriverType.contains("firefox") || 
						strGlobalDriverType.contains("default") || 
						strGlobalDriverType.isEmpty()
					) {
						// default browser or empty browser type
						REAL_WEB_DRIVER 	= new FirefoxDriver();
					} else if (strGlobalDriverType.contains("chrome")) {
						REAL_WEB_DRIVER 	= new ChromeDriver();
					} else if (strGlobalDriverType.contains("internet explorer")) {
						// REAL_WEB_DRIVER 	= new IEDriver();
						...
						// for any other type of browser which is not supported
						REAL_WEB_DRIVER = new FirefoxDriver();
					}
					
					// accidently if nothing is initiated then use firefox type as default
					if (null == REAL_WEB_DRIVER) {REAL_WEB_DRIVER 	= new FirefoxDriver();}
				}
			}catch(Exception e){
